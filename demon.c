...
int main(int argc, char *argv[]) 
{
    ...
    while (odczytany_bajt = read(zadania, &znak, 1) > 0) //Wczytywanie kolejnych bajtów dopóki nie skończy się plik
    {
        if(znak == '\n') //Jeżeli wykryto znak "\n", to znaczy że wiersz się kończy i można go zapisywać do tablicy
        {
            linia[i] = '\0'; //Wczytana linia tekstu zakończona jest znakiem '\0'
            i=0; //Zerujemy zmienną 'i', aby można jej było użyć przy kolejnym przejściu pętli

            wartosc = strtok(linia, ":"); //Wczytywanie pierwszego parametru; Funkcja strtok wczytuje tekst do momentu napotkania dwukropka
            zadania_tab[ilosc_zadan][0] = atoi(wartosc); //Atoi zamienia char na int; Pod zerowy indeks przypisujemy parametr pierwszy - <hour>
            
            wartosc = strtok(NULL, ":"); //Wczytywanie drugiego parametru
            zadania_tab[ilosc_zadan][1] = atoi(wartosc); //Pod pierwszy indeks przypisujemy parametr drugi - <minutes>
            
            wartosc = strtok(NULL, ":"); //Wczytywanie trzeciego parametru
            zadania_tab[ilosc_zadan][2] = atoi(wartosc); //Pod drugi indeks przypisujemy parametr trzeci - <command>
            //Ten sam parametr przypisujemy też do tablicy komend w celu zapewnienia poprawnych wartości
            strcpy(komendy[ilosc_zadan], wartosc); //Kopiowanie wartości trzeciego parametru do tablicy komend za pomocą funkjci strcpy
            
            wartosc = strtok(NULL, ":"); //Wczytywanie czwartego parametru
            zadania_tab[ilosc_zadan][3] = atoi(wartosc); //Pod trzeci indeks przypisujemy parametr czwarty - <info>
            
            ilosc_zadan++; //Zwiększamy wartość zmiennej w celu wczytania kolejnego zestawu parametrów zadania do wykonania
        }
        else
        {
            linia[i++] = znak; //Dla kolejnych wartości zmiennej 'i' przypisujemy kolejne wczytywane znaki dla tablicy przechowującej kolejne wczytywane linie z pliku
        }
    }
    ...
    int zadania_tab2[1000][4]; //Tablica zadań, która bedzie przechowywała przefiltrowane zadania
    char komendy2[1000][100]; //Tablica komend przechwująca przefiltrowane komendy
